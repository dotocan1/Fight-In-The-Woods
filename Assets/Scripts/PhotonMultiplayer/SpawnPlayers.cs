    ...
    void Update()
    {
        if (PlayerPrefab == null && myTeam != 0)
        {
            if (myTeam == 1)
            {
                ...
                if (view.IsMine)
                {
                    PlayerPrefab = PhotonNetwork.Instantiate("Characters/GoodMageCharacter", GameManager.GM.spawnPointsTeamOne[spawnPicker].position, Quaternion.Euler(0, degrees, 0));
